...
def set_user_avatar():
    ...
    if image_file is None:
        # 用户没有上传头像
        return jsonify(errno=RET.PARAMERR , errmsg="为上传头像")
...
def change_user_name():
    """修改用户名"""
    # 使用login_required装饰器后，可以从g对象中获取用户的user——id
    user_id = g.user_id
    ...
    if not name:
        return jsonify(errno=RET.PARAMERR,errmag="名字不能为空")

    # 保存用户的昵称name，并同时判断name是否重复（利用数据库的唯一索引）
    try:
        ...
        current_app.logger.error(e)
        db.session.rollback()
        return jsonify(errno=RET.PARAMERR, errmag="设置用户名错误")

    # 修改session数据中的name字段
    session["name"] = name
    return jsonify(errno=RET.OK, errmag="设置成功",data={"name":name})
